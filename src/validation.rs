...
pub fn validate_title_length(username: &str) -> Result<(), ValidationError> {
    // Ensure that username is not empty before checking length
    let len = username.len();
    match validate_length(len,1,20,"title too short","title too long","title cannot be empty") {
        Ok(_) => return Ok(()),
        Err(res) => return Err(res),
    };
}
...
pub fn validate_content_length(username: &str) -> Result<(), ValidationError> {
    // Ensure that username is not empty before checking length
    let len = username.len();
    match validate_length(len,1,20,"content too short","content too long","content cannot be empty") {
        Ok(_) => return Ok(()),
        Err(res) => return Err(res),
    };
}
